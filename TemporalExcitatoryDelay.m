function [w_input_rec, w_ex_rec] = TemporalExcitatoryDelay(A_LTP, A_LTD)
...
for time=1:10000*100
    ...
    for pre=1:500
        ...
        for ex_neu=1:10
            [ w_input_ex(pre,ex_neu) ] = synapse(w_input_ex(pre,ex_neu), u_ex(ex_neu), umean_plus_ex_d(ex_neu), umean_minus_ex_d(ex_neu), u_bar_bar_ex_d(ex_neu), x_bar_inp(pre), spike_train(pre,mod(time-1,20)+1), syn_par_inp);
        end
    end
    ...
    for pre_ex=1:10
        for post_ex=1:10
            if pre_ex~=post_ex
                ...
                [ w_ex_ex(pre_ex,post_ex) ] = synapse(w_ex_ex(pre_ex,post_ex), u_ex(post_ex), umean_plus_ex_d(post_ex), umean_minus_ex_d(post_ex), u_bar_bar_ex_d(post_ex), x_bar_ex(pre_ex), X_ex(pre_ex), syn_par_ex );
            end
        end
    end
