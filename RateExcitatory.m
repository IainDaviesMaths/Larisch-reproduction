function [w_input_rec, w_ex_rec] = RateExcitatory(A_LTP,A_LTD)
%Rate Excitatory
...
for time=1:10000*100
    ...
    for pre=1:500
        ...
        for ex_neu=1:10
            [ w_input_ex(pre,ex_neu) ] = synapse(w_input_ex(pre,ex_neu), u_ex(ex_neu), umean_plus_ex_d(ex_neu), umean_minus_ex_d(ex_neu), u_bar_bar_ex_d(ex_neu), x_bar_inp(pre), spike_train(pre,mod(time-1,100)+1), syn_par_inp );
        end
    end
