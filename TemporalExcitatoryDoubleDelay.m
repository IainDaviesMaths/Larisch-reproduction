function [w_input_rec, w_ex_rec] = TemporalExcitatoryDoubleDelay(A_LTP, A_LTD)
...
for time=1:10000*100
    ...
    for pre=1:500
        ...
        for ex_neu=1:10
            [ w_input_ex(pre,ex_neu) ] = synapse(w_input_ex(pre,ex_neu), u_ex(ex_neu), umean_plus_ex_dd(ex_neu), umean_minus_ex_dd(ex_neu), u_bar_bar_ex_dd(ex_neu), x_bar_inp(pre), spike_train(pre,mod(time-1,20)+1), syn_par_inp);
        end
    end
